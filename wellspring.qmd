...
## Data Summary: Variables {.smaller}
...
## Data Summary: Variables: Continue {.smaller}
...
## Research Question 1: Method: Continue {.smaller}

-   **Implementation**
    -   Standard regression techniques in **R** (`lm()` function).
    -   Model interpretation: "Holding other factors constant, increasing membership duration by X months increases service attendance by Y."
...
## Research Question 2: Why It Matters

-   **Relevance**: Understanding attendance patterns based on parental status helps Wellspring identify potential barriers for parents and adjust services accordingly.
-   Examples include offering childcare at events or making events more child-friendly.
...
data_summary %>%
  ...
  kable_styling() %>%
  row_spec(which(data_summary$`Parent Status` == "Unknown"), background = "#e5ebf0") %>%
  row_spec(which(data_summary$`Parent Status` == "Unknown"), color = "#20303d")
```
...
## Research Question 2: Hypothesis Test {.smaller}
...
## Research Question 2: Summary {.smaller}
...
## Data/graph Interpretation: Continue {.smaller}
...
## Limitations of the Data {.smaller}

-   **Limited Variables:** <span style="font-size: 0.72em;">
...
## Limitations of the Data: Continue {.smaller}
...
## Recommendations / Next Steps: {.smaller}
